    ...
    try:
        ...
        if not missing_fields:
            if st.button("🚀 Processar no Piperun", type="primary"):
                ...
                try:
                    st.info("🔧 Inicializando processador...")
                    processor = LeadProcessor()
                    
                    status_text.text("Processando...")
                    progress_bar.progress(50)
                    
                    st.info(f"🏷️ Tag: {tag_input if tag_input else 'Nenhuma'}")
                    
                    # Buscar ou criar origem se especificada
                    origin_id = None
                    st.info(f"🌍 Origem: {origin_name if origin_name else 'Nenhuma'}")
                    if origin_name:
                        try:
                            st.info("🔍 Buscando origem...")
                            from piperun_client import PiperunClient
                            client = PiperunClient()
                            
                            # Busca origem existente
                            origins = client.get_origins()
                            st.info(f"📊 {len(origins)} origens encontradas")
                            
                            for origin in origins:
                                if origin['name'] == origin_name:
                                    origin_id = origin['id']
                                    st.success(f"✅ Origem encontrada: {origin_name} (ID: {origin_id})")
                                    st.info(f"🌍 Origem ID: {origin_id}")
                                    break
                            
                            # Se não encontrou, cria nova
                            if not origin_id:
                                st.info(f"➕ Criando nova origem: {origin_name}")
                                new_origin = client.create_origin(origin_name)
                                if new_origin:
                                    origin_id = new_origin['id']
                                    st.success(f"✅ Origem criada: {origin_name} (ID: {origin_id})")
                                    st.info(f"🌍 Origem ID: {origin_id}")
                        except Exception as e:
                            st.error(f"❌ Erro ao processar origem: {str(e)}")
