        ...
        for _, row in df.iterrows():
            ...
            if company and company != 'nan':
                ...
                if column_mapping.get('primeiro_nome') and column_mapping.get('ultimo_nome'):
                    ...
                    if primeiro and primeiro != 'nan' and ultimo and ultimo != 'nan':
                        nome_completo = f"{primeiro} {ultimo}"
                    elif primeiro and primeiro != 'nan':
                        nome_completo = primeiro
                    elif ultimo and ultimo != 'nan':
                        nome_completo = ultimo
                
                # Se não tem First/Last Name, usa campo nome normal
                if not nome_completo and column_mapping.get('nome'):
                ...
                if nome_completo and nome_completo != 'nan':
                    ...
                    # Verifica se já existe esse contato na empresa (evita duplicatas)
                    existing_contact = None
                    for existing in grouped[company]:
                        if existing['nome'].lower() == nome_completo.lower():
                            existing_contact = existing
                            break
                    
                    if existing_contact:
                        # Atualiza contato existente com mais informações
                        if all_emails and not existing_contact['email']:
                            existing_contact['email'] = all_emails
                        if all_phones and not existing_contact['telefone']:
                            existing_contact['telefone'] = all_phones
                    else:
                        # Pega LinkedIn da pessoa
                        linkedin_url = str(row.get('Person Linkedin Url', '')).strip()
                        ...
                        contact = {
                            ...
                            'empresa': company,
                            'linkedin': linkedin_url if linkedin_url and linkedin_url != 'nan' else ''
                        }
                        grouped[company].append(contact)
        
        return dict(grouped)
    
    def process_leads(self, file_path, tag=None, tags=None, origin_id=None):
        ...
        for company, contacts in grouped_leads.items():
            ...
            if created_persons:
                ...
                # Cria oportunidade com nome específico
                deal_title = f"DG - {company}"
